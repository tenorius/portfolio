...
class logo extends Component {
  ...
  handleAnimation = () => {
    const lines = this.svgRef.querySelectorAll('#layer1 > polyline');
    const lines2 = this.svgRef.querySelectorAll('#layer2 > polyline');

    this.animations.code = new TimelineMax();

    this.animations.cursor = new TimelineMax({ yoyo: true, repeat: -1 });

    const interval = 0.4;

    this.animations.code.to(lines, 0, { drawSVG: '0%' });
    this.animations.code.to(lines2, 0, { drawSVG: '0%', strokeOpacity: 1 });
    this.animations.code.to(lines2[0], 0, { drawSVG: '1%' });

    for (let i = 0; i < lines.length; i++) {
      const size = lines[i].points[2].x - lines[i].points[0].x;
      if (size > 45) {
        this.animations.code.to(lines[i], interval, { drawSVG: '100%' })
          .to(lines2[i], interval, { drawSVG: '99% 100%' }, `-=${interval}`);
        if (i === 5) {
          this.animations.code.to(lines[i], interval / 2, { drawSVG: '0%' })
            .to(lines2[i], interval / 2, { drawSVG: '0% 1%' }, `-=${interval / 2}`)
            .to(lines[i], interval, { drawSVG: '100%', stroke: '#e4b723' })
            .to(lines2[i], interval, { drawSVG: '99% 100%' }, `-=${interval}`);
        }
      } else if (size > 20) {
        this.animations.code.to(lines[i], interval / 2, { drawSVG: '100%' })
          .to(lines2[i], interval / 2, { drawSVG: '99% 100%' }, `-=${interval / 2}`);
      } else {
        this.animations.code.to(lines[i], interval / 3, { drawSVG: '100%' })
          .to(lines2[i], interval / 3, { drawSVG: '99% 100%' }, `-=${interval / 3}`);
      }
    }

    this.animations.code.to([lines[10], lines[11]], interval / 1.5, { y: -32, scale: 1.5 });
    this.animations.code.to([lines[10], lines[11]], interval / 1.5, { y: -84, scale: 1 });
  ...
  render() {
    const { size, opacity } = this.props;
    return (
      <Styled.wrapper className="logo" size={size} opacity={opacity}>
        <svg viewBox="0 0 196 200" width="100%" height="auto" id="bgSVG" ref={(ref) => { this.svgRef = ref; }}>
          <title>t logo</title>
          <g style={{ zIndex: 0 }}>
            <title>Layer 1</title>
